...
class AtlasJSONArrayTests: XCTestCase {
    ...
    func testJSONDictionaryParsing() {
        let users: [User]
        do {
            users = try Atlas(TestJSON.jsonArray).mapArray()!
        } catch let e {
    ...
    func testKeyNotInJSONErrorHandling() {
        ...
        var user: [User]?
        do {
            user = try Atlas(TestJSON.jsonArrayMissingKey).mapArray()!
    ...
    func testNoMappingKeyProvidedInModelErrorHandling() {
        ...
        var user: [UserNoKey]?
        do {
            user = try Atlas(TestJSON.jsonArray).mapArray()!
        } catch let e as MappingError {
            switch e {
    ...
    func testNotAnArrayErrorHandling() {
        ...
        var user: [User]?
        do {
            user = try Atlas(TestJSON.user).mapArray()!
        } catch let e as MappingError {
            switch e {
            case .NotAnArray:
                message = "NotAnArray"
    ...
    func testPerformanceExample() {
        self.measureBlock {
            do {
                let _: [User] = try Atlas(TestJSON.jsonArray).mapArray()!
            } catch let e {
                XCTFail("Mapping error occurred: \(e)")
            }
        }
